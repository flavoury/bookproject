...
class recyclerpurchased (private var bookslist : ArrayList<booksItem>) : RecyclerView.Adapter<recyclerpurchased.viewHolder>(),
        Filterable {
            ...
            override fun performFiltering(constraint: CharSequence?): FilterResults {
                ...
                if (charSearch.isEmpty() || charSearch == null) {
                    ...
                    for (row in bookslist) {
                        if (row.getname().toLowerCase(Locale.ROOT).contains(charSearch.toLowerCase(
                                        Locale.ROOT)) || row.getlanguage().toLowerCase(Locale.ROOT).contains(charSearch.toLowerCase(
                                        Locale.ROOT))) {
                            resultList.add(row)
                        }
                    }
